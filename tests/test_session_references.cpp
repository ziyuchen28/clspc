...
{
    const fs::path root =
        fs::temp_directory_path() / "clspc-test-session-references";
    ...
    touch_file(a_file, "class A { int x; int f() { return x; } }\n");
    touch_file(b_file, "class B { int g(A a) { return a.f(); } }\n");
...
while True:
    ...
    const std::vector<Location> refs =
        session.references(a_file, pos, false);

    require(refs.size() == 2, "expected two references");
    require(refs[0].path == fs::absolute(a_file).lexically_normal(),
            "unexpected first reference path");
    require(refs[1].path == fs::absolute(b_file).lexically_normal(),
    ...
    require(contains(log, "\"includeDeclaration\":false"),
            "expected includeDeclaration=false in references request");
    require(contains(log, "\"line\":0"),
            "expected line in references request");
    require(contains(log, "\"character\":22"),
            "expected character in references request");

    std::cout << "\n=== references ===\n";
    print_locations(std::cout, refs);

    fs::remove_all(root, ec);

    std::cout << "test_session_references passed\n";
    return 0;
}
