...
Session Session::spawn(
    ...
    SessionOptions options)
{
    auto transport = pcr::ipc::StdioJsonRpcTransport::spawn(args);
...
std::vector<DocumentSymbol> Session::document_symbols(const std::filesystem::path &path) 
{
    ...
    const std::string result_text = request_json_raw(
...
std::vector<Location> Session::definition(
    const std::filesystem::path &path, 
    Position pos) 
{
    const auto abs = std::filesystem::absolute(path).lexically_normal();
...
std::vector<Location> Session::implementation(
    const std::filesystem::path &path, 
    Position pos) 
{
    const auto abs = std::filesystem::absolute(path).lexically_normal();
...
std::vector<Location> Session::references(
    const std::filesystem::path &path,
    Position pos,
...
std::vector<CallHierarchyItem> Session::prepare_call_hierarchy(
    const std::filesystem::path &path,
    Position pos) 
{
    const auto abs = std::filesystem::absolute(path).lexically_normal();
